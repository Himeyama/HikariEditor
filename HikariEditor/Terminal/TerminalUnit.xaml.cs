...
namespace HikariEditor
{
    public sealed partial class TerminalUnit : Page
    {
        ...
        void StartShell(short cols, short rows)
        {
            ...
            session.Exited += () => QueueWrite("\r\n[プロセスが終了しました]\r\n");

            try
            {
                session.Start(commandLine, workingDir, cols, rows);
            }
            catch (Exception ex)
            {
        ...
        {
            if (webReady)
                dispatcher.TryEnqueue(() => SendToTerminal(base64));
            // webReady になる前（=ready 受信前）は出力は発生しないため捨ててよい
        }
                ...
                switch (type)
                {
                    ...
                    case "input":
                        ...
                        if (data != null && session != null)
                        {
                            ...
                            try
                            {
                                session.Input.Write(bytes, 0, bytes.Length);
                                session.Input.Flush();
                            }
                        ...
                        break;
                    case "resize":
                        session?.Resize(ReadShort(doc, "cols", 80), ReadShort(doc, "rows", 24));
                        break;
                }
            }
            catch { /* 不正な JSON は無視 */ }
